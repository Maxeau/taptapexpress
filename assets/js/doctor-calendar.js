...
$(document).ready(function () {
    ...
    let lastVisible;
    let firstVisible;   
    db.collection('Appointments').where('doctor_id', "==", employee).get().then(function (docs) {
        docs.forEach ( doc => {
    ...
    window.addEventListener('load', function () {
      ...
      function renderEmployee(employee) {
        ...
        let time = ((employee.date_of_birth).seconds) * 1000;
        console.log('date of time', time)
        let dob = (moment(new Date(time)).format("dd D MMM YYYY"));
        time = ((employee.hired_date).seconds) * 1000;
        let hire = (moment(new Date(time)).format("dd D MMM YYYY"));
        console.log('date of birth', dob);
        $("#docteur-id").text('Docteur ID : ' + employee.doctor_id);
        $("#image-url").attr("src", employee.image_url);
        $("#phone-number").text(employee.phone_number);
