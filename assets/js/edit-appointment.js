...
$(document).ready(function () {
    ...
    window.addEventListener('load', function() {
     ...
     function renderAppointment(apt) {
      ...
      function renderPatient(patient) {
        ...
        let dob = moment((patient.date_of_birth).toDate()).format('dd D MMM YYYY', 'en');
    ...
    function saveAppointment() {
        var date = moment(($('#apt-date').val()), "dd DD MMM YYYY HH:mm", "fr");
        console.log('date picked =>', date.toDate())
        let docID = doctor.doctor_id;
        let patientID = patient.patient_id;
        console.log('inside save apt =>', date);
            db.collection('Appointments').doc(uid).update({
                taken_by: employee,
                status: true,
                doctor_id: docID,
                doctor: doctor,
                message: ($('#message').val()),
                apt_date: date.toDate(),
